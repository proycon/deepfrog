...
impl<'a> EditRule<'a> {
    pub fn from_str(editrule: &'a str) -> Result<Self,DeepFrogError> {
            ...
            if mode != ' ' {
                if begin > 0 && c == ']' {
                    instructions.push(match mode {
                        ...
                        _ => return Err(DeepFrogError::OtherError("Parsing editrule failed (invalid mode), should not happen!".to_string()))
                    });
                    //reset
                    begin = 0;
                    mode = ' ';
...
fn compute_lemma(word: &str, editrule: &str) -> Result<String,DeepFrogError> {
    let editrule = EditRule::from_str(editrule)?;
    let mut head: String = word.clone().to_string();
    let mut tail = String::new();

    for instr in editrule.instructions.iter() {
